...
//Read initial configuration
  ...
  {
    ReadConf.open("output/"+ phase + "/config.out");
    ReadVelocity.open("output/"+ phase + "/velocity.out");
  ...
  //tail correction
  vtail = 8.0 * M_PI * rho * ( 1.0/(9.0 * pow(rcut, 9)) - 1.0/(3.0 * pow(rcut, 3)) ) * npart;
  ptail = 32.0 * M_PI * rho * rho * ( 1.0/(9.0 * pow(rcut, 9)) - 1.0/(6.0 * pow(rcut, 3)) ) * 3 * npart;
...
//Print initial values for measured properties
  ...
  cout << "| Initial pressure         = " << rho*temp + (walker[iw])/(3*vol) << endl;
  cout << "'--------------------------------------'" << endl;
...
void Measure() //Properties measurement // ho aggiunto il viriale, per la pressione! <<<<<<<<<<<<<<<<<<
{
  ...
  for(int i=0; i<nbin; i++) gdr[i] = 0; // azzero l'istogramma 
...
// distance i-j in pbc = Min Img Convention
      ...
      bin_index = nbin*(dr/(box/2.));     // posizione nell'istogramma
      if(dr< box/2.){
        gdr[ bin_index ] += 2; //<<<<<<<<<<<< gdr
...
// walker è il vettore che tiene tutte le quantità: le registro
    walker[iv] = 4.0 * v + vtail;                       // Potential energy, vedi slide 43
    ...
    walker[iw] = w + ptail;                 // Pressure: calcolata indirettamente dal viriale. <<<<<<<<<<<<<<<<<<<<
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, Gdr;
    ...
    Gdr.open("output/" + phase + "/output_gdr.dat",ios::app);
    ...
    for(int i=0; i<nbin; i++)         // normalizzo le medie della gdr
    {
      double r = i*(box/2.)/nbin; // distanza tra due particelle
      double dr =  (box/2.)/nbin;  // risoluzione della gdr
